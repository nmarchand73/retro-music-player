...
const OPENMPT_INTERACTIVE_INTERFACE_SIZE = 16 * 4
const OPENMPT_SET_CHANNEL_MUTE_INDEX = 10
...
class MPT extends AudioWorkletProcessor {
	...
	handleMessage_(msg) {
		...
		switch (msg.data.cmd) {
			...
			case 'setOrderRow':
				...
				break
			case 'setChannelMute':
				if (!v || typeof v.channel !== 'number') return
				this.setChannelMute(v.channel, Boolean(v.mute))
				break
			case 'decodeAll':
				this.decodeAll(v)
				break
	...
	play(buffer, paused = false) {
		...
		if (!libopenmpt || !libopenmpt._malloc) {
			this.port.postMessage({cmd:'err',val:'lib'})
			return
		}
		...
		this.moduleExtPtr = libopenmpt._openmpt_module_ext_create_from_memory(ptrToFile, byteArray.byteLength, 0, 0, 0)
		this.modulePtr = this.moduleExtPtr
			? libopenmpt._openmpt_module_ext_get_module(this.moduleExtPtr)
			: 0
		if (!this.modulePtr && libopenmpt._openmpt_module_create_from_memory) {
			if (this.moduleExtPtr) {
				libopenmpt._openmpt_module_ext_destroy(this.moduleExtPtr)
				this.moduleExtPtr = 0
			}
	...
	bindInteractive() {
		if (!this.moduleExtPtr || !libopenmpt.getWasmTableEntry) return false
		if (this.setChannelMuteFn) return true
		const buf = libopenmpt._malloc(OPENMPT_INTERACTIVE_INTERFACE_SIZE)
		const ok = libopenmpt._openmpt_module_ext_get_interface(
			this.moduleExtPtr,
			asciiToStack('interactive'),
			buf,
			OPENMPT_INTERACTIVE_INTERFACE_SIZE,
		)
		if (!ok) {
			libopenmpt._free(buf)
			return false
		}
		const fnPtr = libopenmpt.HEAPU32[(buf >> 2) + OPENMPT_SET_CHANNEL_MUTE_INDEX]
		if (!fnPtr) {
			libopenmpt._free(buf)
			return false
		}
		this.interactiveBuf = buf
		this.setChannelMuteFn = libopenmpt.getWasmTableEntry(fnPtr)
		return true
	}
	setChannelMute(channel, mute) {
		if (!this.bindInteractive()) return
		this.setChannelMuteFn(this.moduleExtPtr, channel, mute ? 1 : 0)
	}
	stop() {
		if (this.interactiveBuf) {
			libopenmpt._free(this.interactiveBuf)
			this.interactiveBuf = 0
			this.setChannelMuteFn = null
		}
		if (this.moduleExtPtr) {
